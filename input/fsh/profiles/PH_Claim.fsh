...
* contained ..0
// * extension ..0
* extension contains EncounterExtension named encounter 0..*
* modifierExtension ..0
// * identifier ..0
...
* patient only Reference($SD_Patient)    //Can't disable elements that are mandatory by default
* billablePeriod ..0
...
* enterer ..0
* insurer ..0
* provider only Reference($SD_Practitioner or $SD_PractitionerRole or $SD_Organization)   //Can't disable elements that are mandatory by default
// * priority 0..0                  Can't disable elements that are mandatory by default
* fundsReserve ..0
* related ..0
* prescription only Reference(DeviceRequest or $SD_MedicationRequest or VisionPrescription)
* originalPrescription ..0
* payee.party only Reference($SD_Practitioner or $SD_PractitionerRole or $SD_Organization or $SD_Patient or $SD_RelatedPerson)
* payee.id ..0
* payee.extension ..0
* payee.modifierExtension ..0
* referral only Reference($SD_ServiceRequest)
* facility ..0
// * careTeam ..0
* careTeam.responsible ..0
* careTeam.role ..0
* careTeam.qualification ..0
* careTeam.provider only Reference($SD_Practitioner or $SD_PractitionerRole or $SD_Organization)
// * supportingInfo ..0
* supportingInfo.id ..0
...
* diagnosis.diagnosisReference only Reference($SD_Condition)
* diagnosis.type ..0
...
* procedure.type ..0
// * procedure.date ..0
* procedure.procedureReference only Reference($SD_Procedure)   //Can't disable elements that are mandatory by default
* procedure.udi ..0
...
* item ^slicing.discriminator.path = "productOrService.coding.code"
* item ^slicing.rules = #open

// Slice declaration
* item contains
    repetitiveProcedure 0..1 and
    zbenefitPackage 0..1 and
    tbDots 0..1 and
    animalbitePackage 0..1 and
    newborncarePackage 0..1 and
    firstCaseRate 0..1 and
    secondCaseRate 0..1

// Fix slice constraints using codes
* item[repetitiveProcedure].productOrService.coding.code = #repetitive-procedure
* item[zbenefitPackage].productOrService.coding.code = #zbenefit
* item[tbDots].productOrService.coding.code = #tb-dots
* item[animalbitePackage].productOrService.coding.code = #animal-bite
* item[newborncarePackage].productOrService.coding.code = #newborn-care
* item[firstCaseRate].productOrService.coding.code = #first-case-rate
* item[secondCaseRate].productOrService.coding.code = #second-case-rate
* item.id ..0
* item.extension ..0
...
// * total ..0
* item.encounter only Reference($SD_Encounter)