...
* doNotPerform ..0
* reported[x] ..0
* medicationCodeableConcept from $DrugCodesVS (required)
* medicationReference only Reference($SD_Medication)  //  Can't disable elements that are mandatory by default
* subject only Reference($SD_Patient or Group)    //  Can't disable elements that are mandatory by default
